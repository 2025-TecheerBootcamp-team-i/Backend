...
#### 유효성 검증 규칙
- **이메일**: 형식 검증 ※ 중복 검사는 별도 API 사용
- **비밀번호**: 8-16자 + 영문자/숫자/특수기호 각 1개 이상
- **닉네임**: 필수 입력 (2-20자)

**주의사항:**
- 이메일 중복 검사는 회원가입 전에 `GET /api/v1/auth/check-email/` API를 사용하세요
...
**주의사항:**
- 이메일 형식 검증은 프론트엔드에서 수행하므로, 올바른 형식의 이메일만 전달하세요
- 중복 검사만 수행합니다

#### 요청 파라미터
- `email` (필수): 중복 검사할 이메일 주소 (형식 검증된 상태)
...
```
※ 프론트엔드에서 형식 검증을 수행하므로 이 응답은 일반적으로 발생하지 않습니다.
